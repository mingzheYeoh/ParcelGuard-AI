...
1. Install `@terminal3/t3n-sdk@5.10.0` in `apps/api`; run `scripts/preflight/terminal3-smoke.mts`. **Known blocker (task-0.md):** `fetchTrustedManifest('testnet')` fails because the testnet manifest lacks `rtmr1_allowlist`; only `{ unsafe_trust_server: true }` connects. Ask Terminal 3 devrel first; if unresolved, use the unsafe anchor only with `evidence.verified=false` and an explicit "attestation not verified" status. (identity handshake + `authenticate` → `did:t3n:…`). Identity needs no Rust. A protected action needs a registered WASM contract (`tenant.contracts.register/execute`); reuse `Terminal-3/z-tenant-flight` or the Circle demo contract rather than writing Rust from scratch, and only if time allows.
